...
| VitalDB, первоначальная | исследовательский выбор нормализации | 100 случаев | 94 | 9 400 | 0 | 0 |
...
**Интерпретация.** В сопроводительных материалах оригинальной работы оставшиеся 27 260 эпизодов описаны как независимый тестовый набор. Приведённые измерения показывают, что независимость этой части от строк, отводимых под обучение и валидацию, не выполняется на уровне содержимого сегментов. Установить, какие данные использовались при обучении опубликованных весов, по доступным материалам невозможно, поэтому независимость набора не может быть подтверждена. Соответственно, полученные на нём в настоящей работе показатели, включая MAE формы волны 3,680 мм рт. ст., интерпретируются как внутридоменный тест воспроизводимости и оптимистичная граница достижимой точности. На основной вывод работы это не влияет: при поправке на это обстоятельство наблюдаемое ухудшение при переходе к ABP_PPG и VitalDB может оказаться только меньше заявленного, но не больше.
...
| VitalDB, независимая | Ошибка АДср | 12,464 [6,031; 20,779] | 36,472 | 559,783 | 943 | 242 | 137 | 28 | 2 |
...
**Рисунок S1.** Предсказанные и эталонные значения САД и ДАД: полный диапазон исходных результатов без постобработки. Ось эталонных значений линейная, ось предсказаний представлена в симметричном логарифмическом масштабе.

![Предсказанное и эталонное САД и ДАД, увеличенные диапазоны](codes/figures/revision_v2/point5/prediction_reference_zoom.png)

**Рисунок S2.** Предсказанные и эталонные значения: увеличение диапазона [60; 220] мм рт. ст. для САД и [20; 140] мм рт. ст. для ДАД.

![Калибровочные кривые](codes/figures/revision_v2/point5/calibration.png)

**Рисунок S3.** Калибровка САД и ДАД в фиксированных интервалах эталонного давления шириной 20 мм рт. ст. Показаны интервалы, представленные не менее чем в 10 кластерах; пунктирная линия соответствует идеальной калибровке.
...
**Рисунок S4.** Диаграммы Бланда–Альтмана для САД и ДАД: полный диапазон исходных результатов без постобработки.

![Диаграммы Бланда–Альтмана, увеличенный диапазон](codes/figures/revision_v2/point5/bland_altman_zoom.png)

**Рисунок S5.** Диаграммы Бланда–Альтмана для САД и ДАД: увеличение диапазона разностей от −100 до 100 мм рт. ст. Число точек вне показанного диапазона указано над каждой панелью.
...
**Рисунок S6.** Эмпирические функции распределения MAE формы волны по отдельным сегментам без постобработки для MIMIC-III, ABP_PPG, первоначальной и независимой выборок VitalDB (нормализация A). Ось ошибки логарифмическая.
